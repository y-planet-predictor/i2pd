...
namespace data
{
	...
	int Reseeder::ReseedFromSU3Url (const std::string& url, bool isHttps)
	{
	    ...
	    // ”силенна€ проверка на "магические байты"
	    if (su3.size() < 40 || memcmp(su3.data(), "I2Psu3", 6) != 0) 
	    {
	        ...
	        return 0; // ¬озвращаем 0, чтобы цикл ReseedFromServers перешел к следующей ссылке
	    }

	    // ≈сли проверка пройдена, скармливаем поток парсеру
